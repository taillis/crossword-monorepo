...
function parseEntries(xml: string): RawWordNode[] {
  ...
  while ((match = entryRegex.exec(xml)) !== null) {
    ...
    if (category === 'geral') {
      ...
      if (/planta|árvore|flor|folha|fruto|ave|pássaro|peixe|animal|mamífero|inseto|réptil|floresta|erva|arbusto/i.test(lowerClue)) {
        category = 'natureza';
      } else if (/física|química|mineral|átomo|molécula|célula|órbita|planeta|estrela|astronomia|matemática|doença|médic|bactéria|vírus/i.test(lowerClue)) {
        category = 'ciencia';
      } else if (/país|cidade|rio|montanha|capital|ilha|continente|oceano|mar|província|região|relevo|lago|serra/i.test(lowerClue)) {
        category = 'geografia';
      } else if (/antigo|século|guerra|imperador|monarca|dinastia|revolução|grego|romano|medieval|história|império/i.test(lowerClue)) {
        category = 'historia';
      }
    }
...
async function main() {
  ...
  const letters = ['A', 'B', 'C', 'D', 'E', 'F', 'H', 'I', 'J', 'M', 'N', 'O', 'P', 'Q', 'T', 'U', 'V', 'Z'];
